...
const Profileuser = () => {
  ...
  const getDataDashboard = async () => {
    try {
      const response = await axios.get("http://localhost:3000/dev/current", {
        headers: {
          Authorization: `${token}`,
        },
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.patch(
        "http://localhost:3000/dev/current/update",
        passwordData,
        {
          headers: {
            Authorization: `${token}`,
  ...
  return (
    <>
      ...
      <div className="container mx-auto mt-5  justify-center">
        <div className="card bg-slate-600 w-4/4 mx-auto">
          <div className="grid grid-cols-2 gap-5 text-center">
            ...
            <label className="input input-bordered flex items-center gap-2">
              ...
              <p className="text-center">{dataUser.nama}</p>
            </label>
          </div>
        </div>
